...
let firstNamesAllCaps = runners.map(x => x.first_name.toUpperCase());
...
const ticketPriceTotal = function(arr) {
  let result = [];
  for (let i = 0; i < arr.length; i++) {
    result.push(arr[i].donation);
  }
...
// Problem 1
// Change the company name to all lowercase

let companyName = runners.map(x => x.company_name.toLowerCase());
console.log(companyName);

// Problem 2
// Filter out everyone with a name longer than 5 characters

let shortNames = runners.filter(x => x.first_name.length < 5);
console.log(shortNames);

// Problem 3
// Return everyone with a shirt size of M or S
let smallResult = [];
let smallRunners = runners.forEach(function(arr) {
  if (arr.shirt_size === "M" || arr.shirt_size === "S") {
    smallResult.push(arr.first_name);
  }
});
